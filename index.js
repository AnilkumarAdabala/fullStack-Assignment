...
app.post('/addItem', async (request, response) => {
    try {
        ...
        const { title, description, archeive, is_delete } = notesDetails;
        const sqlQuery = `
        INSERT INTO notes
            (title, description, archeive, is_delete) 
        VALUES (?, ?, ?, ?)
    `;

        await db.run(sqlQuery, [title, description, archeive, is_delete]);
...
app.get('/getNotes', async (request, response) => {
    try {
        const sqlQuery = `SELECT * FROM notes  WHERE archeive = 0 AND is_delete = 0;`;
...
app.get('/getArcheive', async (request, response) => {
    try {
        const sqlQuery = `SELECT * FROM notes  WHERE archeive = 1 ;`;
...
app.get('/getisDelete', async (request, response) => {
    try {
        const sqlQuery = `SELECT * FROM notes  WHERE archeive = 0 AND is_delete = 1;`;
...
app.put('/updateArchivedNotes', async (request, response) => {
   
    try {
        const {id, archeive} = request.body
        const sqlQuery = `
        UPDATE 
            notes
        SET
        ...
        const result = await db.run(sqlQuery, [archeive, id]);
